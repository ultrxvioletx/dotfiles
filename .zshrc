...
# --- Prompt Personalizado basado en Rosé Pine ---
# 1. Cargar el módulo de información de control de versiones (para Git)
autoload -Uz vcs_info
precmd() { vcs_info }

# 2. Formatear la información de Git
#    Solo se mostrará si estás en un repositorio Git.
#    El espacio inicial está DENTRO de las comillas para que solo aparezca con el branch.
#    Usamos 'red' que en tu tema es el color "Love" (#eb6f92).
zstyle ':vcs_info:git:*' formats " [%{$fg[red]%}%b%{$reset_color%}]"

# 3. Función para mostrar el entorno virtual de Python (si está activo)
prompt_venv() {
    if [[ -n "$VIRTUAL_ENV" ]]; then
    # Extrae el nombre del entorno de la ruta y lo muestra en color "Rose" (#EBBCBA),
    # que para Zsh es 'green' en tu tema.
